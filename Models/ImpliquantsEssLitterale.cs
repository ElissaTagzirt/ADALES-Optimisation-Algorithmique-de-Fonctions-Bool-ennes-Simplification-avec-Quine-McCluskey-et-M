...
namespace APPLICATION.Models
{
    ...
    public class ImpliquantsEssLitterale
    {
        ...
        public List<int> mintermes = new List<int> { 0, 2, 3, 5, 6, 7, 8, 9, 10, 14, 15, 30000 };
        List<int> copy_min;
        ...
        //-----------------------------------------------------------------le resultat finale-------------------------------------------------------------------------------------
        /***/
        SortedSet<String> Empliquantfinale;

        public ImpliquantsEssLitterale()
        {
            copy_min = new List<int>(mintermes);
            petrick = new string[mintermes.Count];
            Empliquantfinale = new SortedSet<string>();
        }
        //------------------------------------------------------------------------------------------------------------------------------------------------------
        public string[] petrick;
        public string VP = "";
        ...
        SortedSet<String> getEmpliquantfinale() { return Empliquantfinale; }
        ...
        //--------------------------------------------------------------------------
        public void importpremiers(ExpressionLet f)
        {
            Dictionary<ImpPrem, char> d = new Dictionary<ImpPrem, char>();
            List<TabTermes> l = new List<TabTermes>(f.gettabTermesPremie());
            ImpPrem s = new ImpPrem();
        ...
        public string ReductionTotaleQMCandPETRIK()
        {
            ...
            AffichageDesResultatFinale();
            Console.WriteLine("active value " + ExpressionLet.activeLet);
            if (ExpressionLet.activeLet == 1)
            {
                Console.WriteLine("i am her " + ExpressionLet.activeLet);
                Console.WriteLine("Le resultat finale " + ConverterMintermeLet(Empliquantfinale));
            }
            else { Console.WriteLine("Le resultat finale " + ConverterMintermeNUM(Empliquantfinale)); }

            return ConverterMintermeLet(Empliquantfinale);
        }
        //--------------------------------------------------------------------------------------------------------------------------------------------------//
        public short reduc()
        ...
        //-------------------------------------------------------------------------------------------------------------
        public string ConvertesseurBinToStringForXLet(string mintemeBin)
        {
            string resultat = "";
            byte stop = 0;
            while (stop != 1)
            {
                ...
                for (int i = 0; i < mintemeBin.Length; i++)
                {
                    ...
                    if (c != 'x')
                    {
                        string str = ConverteStringLet(mintemeBin[i], ExpressionLet.getVarPos(ASCCI));
                        resultat = resultat + str;
        ...
        //_________________________________________________________________________________________________________________________________
        public string ConverteStringLet(char var, string ascii)
        {

            if (var != 'x')
            {
                if (var == '0') { return "!" + ascii + "."; }
                else
                {
                    if (var == '1') { return ascii + "."; }
                }
        ...
        public string ConverterMintermesansXLet(string impl, int tailleImpl)
        {
            ...
            if ((impl.Length - tailleImpl) != 0)
            {
                ...
                return ConvertesseurBinToStringForXLet(Impliquant);

            }
            else
            { return ConvertesseurBinToStringForXLet(impl); }

        }
        //----------------------------------------------------------------------------------------------------------------------------------
        public string ConverterMintermeNUM(SortedSet<String> impls)
        {
            string result = "";
            //impls.Reverse();
        ...
        //---------------------------------------------------------------------------------------------------------
        public string ConverterMintermeLet(SortedSet<String> impls)
        {
            string result = "";
            foreach (string impl in impls)
            { result = result + "+" + ConvertesseurBinToStringForXLet(impl); }
        ...
        public short EstILessentielle(string A)
        {
            for (int i = 0; i < A.Length; i++)
            {
                if (A[i] == 'x') { return 1; }
        ...
        public void CleaninglisteFinale(SortedSet<String> A)
        {
            int i = 0;
            while ((i < A.Count) && (A.Count != 0))
            {
                ...
                {
                    short result = egaliteMinterme(A.ElementAt(i), A.ElementAt(j));
                    if (result == 1) { A.ElementAt(j); }
                    else { j++; }//cette element existe deja
                }
                i++;
            }
            foreach (string impl in A) { Console.WriteLine("var :" + impl); }
        }

    }
}// fin de classe ImpliquantsEss
